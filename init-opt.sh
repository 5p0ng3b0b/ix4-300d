...
# Wait until internal HDD storage pool is mounted
while [ ! "$(ls -a /mnt/pools/A/A0/ 2>/dev/null)" ]; do sleep 2; done
# Set drive for location of Optware and apps
# HDD is internal HDD storage pool USB is external drive
# If USB then must specify UUID of drive
OPTDRV=HDD; UUID="0"
# Wait for external USB drive if selected as optware volume
[ "$OPTDRV" = "USB" ] && while [ ! "$(blkid | grep "$UUID")" ]; do sleep 2; done

# Assign variables and define functions
[ "$OPTDRV" = "USB" ] && OPT=$(mount | grep "$(blkid | grep "$UUID" | awk -F ':' '{print $1}') on /mnt" | awk '{print $3}' | grep -v '/opt')
...
# Use path defined in $OPT for /opt to prevent running out of space
while true; do umount /opt 2>/dev/null; if [ ! $? -eq 0 ]; then break; fi; done
mkdir -p "$OPT"; mount --bind "$OPT" "/opt"
# Mount /home folder in optware volume
while true; do umount /home 2>/dev/null; if [ ! $? -eq 0 ]; then break; fi; done
mkdir -p "$OPT/home"; mount --bind "$OPT/home" "/home"
# Use Optware volume for /var/tmp to prevent running out of space
while true; do umount /var/tmp 2>/dev/null; if [ ! $? -eq 0 ]; then break; fi; done
mkdir -p "$OPT/var/tmp"; mount --bind "$OPT/var/tmp" "/var/tmp"
...
# Install or fix Optware-ng if required. This will also run ipkg update each reboot.
if [ ! -f "/opt/bin/ipkg" ] || [ ! "$(ipkg update)" ]; then
...
# /opt/etc/profile is executed via $HOME/.profile script
# If you mess up /opt/etc/profile just delete it and reboot to restore it
# Some packages install the optware version of bash which overwrites it
# Ideally all your modifications to paths etc should go in $HOME/.profile
# /opt/etc/profile. Do not move or change but delete it if need be
#begin_profile
profile=\
...
# Now we source $HOME/.profile to setup paths
. "$HOME/.profile"
...
[ -f "/mnt/pools/A/A0/init-user.sh" ] && nohup sh user-init.sh >/dev/null 2>&1 &
